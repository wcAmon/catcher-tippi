...
> **已知問題(截至本文件撰寫時,尚未修復,請先讀完再繼續)**:目前
> `manifest.json` pin 住的 macOS engine host release
> (`asr-host-v0.1.0`)存在一個上游打包缺陷——執行時會印出
> `MLX error: Failed to load the default metallib. library not found`
> 然後退出。這**不是**你操作錯誤,也不是這份配方的 Deno 程式碼有問題
> (`verify:mac` 五個測試套件本身在替換成一份功能正常的同源 binary 後
> 全數通過,見 `task-4-report.md` 的完整證據)。如果你在 Step 3(setup)
> 之後的 Step 4(verify)遇到這個確切錯誤訊息,代表這個上游缺陷還沒修好
> ——**停下來,回報這個發現,不要自己嘗試修 Rust 原始碼或猜測 workaround**
> (那超出配方組裝 agent 的職責範圍)。Windows engine host 目前未知是否
> 有同類問題,Task 6 演練會確認。
...
running 11 tests from ./verify/asr_metric_test.ts
（11 個全部 ok）
...
ok | 17 passed (12 steps) | 0 failed
```
...
| 症狀                                                                         | 原因                                                                      | 解法                                                                                    |
| ---------------------------------------------------------------------------- | ------------------------------------------------------------------------- | --------------------------------------------------------------------------------------- |
| `MLX error: Failed to load the default metallib. library not found`(mac)     | 見本文件最開頭的「已知問題」——目前 pin 住的 release binary 有上游打包缺陷 | 停下來回報這個確切錯誤訊息,不要自行嘗試修復                                             |
