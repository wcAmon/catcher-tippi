> 本檔為 `docs/protocol/asr-host-v1.md` 的隨包副本,以源檔為準。
...
| partial | `{"event":"partial","text":"..."}`  | 會話累積轉錄的最新全文(非增量)。僅在有新 token 時輸出。連續兩則內容相同的 partial 是允許的;client 必須把每則 partial 視為完整快照替換,而非增量。                                  |
| final   | `{"event":"final","text":"..."}`    | stop 之後的定稿全文,一個會話恰好一次。                                                                                                                                            |
| error   | `{"event":"error","message":"..."}` | 可恢復錯誤(格式錯、狀態錯)。行程不退出;致命錯誤才退出(exit 1)。`error.message` 為人類可讀、host-specific 的文字,兩個 host 的措辭不保證一致;client 不得解析或分支於 message 內容。 |
